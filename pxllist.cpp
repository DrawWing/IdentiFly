...
void 
pxlList::deLoop(nodePxl* currPxl, unsigned loopCnt){
  
  unsigned newIndex=0;
  
  nodePxl* loopPxl = currPxl->next;      
  do{
    newIndex++;
    if(currPxl->pxl==loopPxl->pxl){
      if(loopCnt < 3){
        if(newIndex<(size-newIndex)){
          for(nodePxl* loopIter=loopPxl;loopIter!=currPxl;){
            nodePxl* temp=loopIter;
            loopIter=loopIter->prev;
            erase(temp);
          }
          break;
        }else{
    ...
    loopPxl=loopPxl->next;
  }while(loopPxl != currPxl);
}

void 
pxlList::deLoop1(nodePxl* currPxl){
...
//remove parts of outline stickin togather
void 
pxlList::proon()
{
  if(size<4)//zmienic
    return;
//  bool progress = true; //progress nie potrzebny 1 petla powinna wystarczyc
  //  while(progress){
//  progress = false;
  nodePxl* currNode = front; //wprowadzic currNode++ 
  do{
    nodePxl* prevNode = currNode->prev;
    nodePxl* nextNode = currNode->next;
    
    if(currNode->pxl == nextNode->pxl){
      erase(nextNode);
      currNode=prevNode;
//      progress = true;
      if(size<4)
        return;
    }else if(prevNode->pxl == nextNode->pxl){
        erase(currNode);
        erase(nextNode);
        currNode=prevNode->prev;//befor previous to finis in one go
//        progress = true;
        if(size<4)
          return;
    }//mozna dac tu else currNode++ i zmienic zmiany powyzej
    currNode=currNode->next;
  }while(currNode != front); //do
}
